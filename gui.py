...
class GUI:
    def __init__(self, master):
        ...
        self.user_drawing = Image.new("RGB", (800, 600), "white")
        self.action_stack = []
        ...
        # Добавляем кнопку для удаления белого фона
        self.button_remove_background = tk.Button(master, text="Remove Background", command=self.remove_white_background)
        self.button_remove_background.pack(side=tk.TOP, pady=2)

    def remove_white_background(self):
        if self.original_image:
            processed_image = remove_white_background(self.original_image)
            self.original_image = processed_image
            self.user_drawing = processed_image.copy()
            self.display_image()
    ...
    def undo(self, event):
        if self.action_stack:
            self.action_stack.pop()  # Убираем последнее действие
            if self.action_stack:
